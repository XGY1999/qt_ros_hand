...
class calibration:
    def __init__(self):
        ...
        self.bin_num = 90
        self.abe_array = np.load('/home/xgy/XGY/xgy_git/ros_hand/qt_ros_hand/src/gelsight/src/abe_corr.npz') # change this with your aberration array绝对路径
        self.x_index = self.abe_array['x']
        self.y_index = self.abe_array['y']
    

    def crop_image(self, img, pad): # 这里剪切之后出现bug，显示之后处理数组大小不匹配，所以直接删除crop部分，返回输入
        return img
...
if __name__=="__main__":
    cali = calibration()
    pad = 20
    ref_img = cv2.imread('/home/xgy/XGY/xgy_git/ros_hand/qt_ros_hand/src/gelsight/test_data/ref.jpg')
